...
assoc-3 : ∀ {A B C D E} {f : B ⇒ A} {g : C ⇒ B} {h : D ⇒ C} {i : E ⇒ D} → 
  (f ∘ g ∘ h) ∘ i ≈ f ∘ g ∘ h ∘ i
assoc-3 = assoc ∙ ∘-resp-≈ʳ assoc

assoc-4 : ∀ {A B C D E F} {f : B ⇒ A} {g : C ⇒ B} {h : D ⇒ C} {i : E ⇒ D} {l : F ⇒ E} → 
  (f ∘ g ∘ h ∘ i) ∘ l ≈ f ∘ g ∘ h ∘ i ∘ l
assoc-4 = assoc ∙ ∘-resp-≈ʳ assoc-3

sym-assoc-3 : ∀ {A B C D E} {f : B ⇒ A} {g : C ⇒ B} {h : D ⇒ C} {i : E ⇒ D} → 
  f ∘ g ∘ h ∘ i ≈ (f ∘ g ∘ h) ∘ i
sym-assoc-3 = Equiv.sym assoc-3

sym-assoc-4 : ∀ {A B C D E F} {f : B ⇒ A} {g : C ⇒ B} {h : D ⇒ C} {i : E ⇒ D} {l : F ⇒ E} → 
  f ∘ g ∘ h ∘ i ∘ l ≈ (f ∘ g ∘ h ∘ i) ∘ l
...
skip-2 : ∀ {A B C D} {f : B ⇒ A} {g : C ⇒ B} {z z′ : D ⇒ C} → z ≈ z′ → 
  f ∘ g ∘ z ≈ f ∘ g ∘ z′
skip-2 eq = skip-1 (skip-1 eq)

skip-3 : ∀ {A B C D E} {f : B ⇒ A} {g : C ⇒ B} {h : D ⇒ C} {z z′ : E ⇒ D} → z ≈ z′ → 
  f ∘ g ∘ h ∘ z ≈ f ∘ g ∘ h ∘ z′
...
sym-id-1 = Equiv.sym identityʳ

id-1 : ∀ {A B C} {f : B ⇒ C} {g : A ⇒ B} → f ∘ id ∘ g ≈ f ∘ g
id-1 = skip identityˡ
