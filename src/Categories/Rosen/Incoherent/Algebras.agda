...
--   α = [ f , Φ# ]
--
-- No braiding is involved: Φ already lives in [ A , B ]₀, so transposing
  ...
  ; F₁ = λ f →
    let module f = iMR2ᴿ⇒ f
        ...
        yoga = begin
            ...
            adjoint.Radjunct (f.ξY.Φ ∘ f.v)           ≈⟨ refl⟩∘⟨ ⊗A.homomorphism ○ sym-assoc ⟩
            ΦY# ∘ (f.v ⊗₁ idA)                        ∎
      in record
      { f = f.v
      ; commutes =
...
--   Φ = Ladjunct (α ∘ i₂)
--
-- i.e. transpose the map α ∘ i₂ : A ⊗ B ⇒ B through the closed structure.
