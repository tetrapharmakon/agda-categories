...
module Categories.Rosen.Incoherent.HigherMRS {o ℓ e} {C : Category o ℓ e} {M : Monoidal C} (Cl : Closed M) where
...
-- iMRS3: the 3rd level, IsoComma of ℝ (from ProElements) and V₁ (from Tabulator).
iMRS3 : Category (o ⊔ ℓ ⊔ e) (o ⊔ ℓ ⊔ e) e
iMRS3 = IsoComma ℝ [_]f

-- 𝕚𝕄ℝ𝕊 n: the n-th level category together with a functor to Arr.Arrow.
𝕚𝕄ℝ𝕊 : (n : ℕ) → Σ (Category (o ⊔ ℓ ⊔ e) (o ⊔ ℓ ⊔ e) e) (λ x → Functor x Arr.Arrow)
𝕚𝕄ℝ𝕊 zero = iMRS3 , V₂
  where 
    -- Base level: functor V₂ from iMRS3 to Arr.Arrow.
    V₂ : Functor iMRS3 Arr.Arrow 
    V₂ = {!   !} --  record
      -- { F₀ = λ x → 
...
-- 𝕚𝕄ℝ𝕊ₐ n: the functor from the n-th level to Arr.Arrow.
-- 𝕚𝕄ℝ𝕊ₐ : (n : ℕ) → Functor (𝕚𝕄ℝ𝕊ₒ n) Arr.Arrow
-- 𝕚𝕄ℝ𝕊ₐ n = proj₂ (𝕚𝕄ℝ𝕊 n)
...
𝕚𝕄ℝ𝕊-down : ∀ {n m} → m ≤ n → Σ (Functor (𝕚𝕄ℝ𝕊ₒ n) (𝕚𝕄ℝ𝕊ₒ m)) (λ F-down → NaturalIsomorphism (V m ∘F F-down) (V n))
...
lemma-id : ∀ {n : ℕ} → NaturalIsomorphism (downF {n} {n} ≤-refl) (idF {C = 𝕚𝕄ℝ𝕊ₒ n})
...
-- lemma-homomorphism: 𝕚𝕄ℝ𝕊-down respects composition up to natural isomorphism.
...
lemma-homomorphism {n = n} z≤n z≤n = niHelper (record
  { η = λ X → M0.id
  ; η⁻¹ = λ X → M0.id
  ; commute = λ f → id-comm-sym (𝕚𝕄ℝ𝕊ₒ zero) {f = Functor.F₁ (reduce n) f}
...
-- MRS-chain: the chain ... → 𝕚𝕄ℝ𝕊ₒ 2 → 𝕚𝕄ℝ𝕊ₒ 1 → 𝕚𝕄ℝ𝕊ₒ 0 as ℕ^op → Cats. (WIP: has holes)
...
MRS∞ = MRS-Limit.apex
-- MRS∞-proj: projection functors MRS∞ → 𝕚𝕄ℝ𝕊ₒ n.
